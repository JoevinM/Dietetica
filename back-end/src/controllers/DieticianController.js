...
class DieticianController {
  ...
  async getFirst(req, res, next) {
    try {
      // Récupère le premier diététicien disponible
      const dietician = await dieticianRepository.findFirst({});

      if (!dietician) {
        return res.status(404).json({ message: "Aucun diététicien disponible." });
      }

      // On retire le mot de passe avant d'envoyer
      res.json(dieticianRepository.excludePassword(dietician));
    } catch (err) {
      next(err);
    }
  }
