...
/**
 * Construit une datetime ISO à partir d'une date et d'une heure
...
/**
 * Extrait l'heure "HH:00" depuis une dateTime ISO ou string
...
/**
 * Formate une date ISO en label lisible français
 * @param {string} dateStr - "YYYY-MM-DD"
 * @returns {string}
 */
...
export default function BookAppointment() {
  ...
  // Statut de la connexion Google Calendar (true = connecté, false = non connecté, null = inconnu)
  ...
  // ─── Initialisation : décodage du token + chargement du diété ─────────────

  useEffect(() => {
    async function initPage() {
      // 1. Décoder le JWT pour savoir qui est connecté
      const jwtToken = getTokenFromCookie();
      if (jwtToken) {
        const payload = decodeJwtPayload(jwtToken);
        setCurrentUser(payload); // contient { id, role }
      }
      ...
      // 3. Vérifier si Google Calendar est connecté en testant la route
  ...
  /**
   * Tente de lister les events Google Calendar pour savoir si le token OAuth est actif.
   * Si la réponse est OK → connecté. Sinon → non connecté.
   */
  async function checkGoogleCalendarStatus() {
    try {
      const response = await fetch(`${API_BASE_URL}/calendar`);
      // Si la réponse est OK, Google Calendar est opérationnel
  ...
  // ─── Connexion Google Calendar (rôle diététicien/admin uniquement) ─────────

  /**
   * Redirige le diététicien vers l'URL d'auth Google OAuth2
   */
  async function handleConnectGoogle() {
    try {
      ...
      // Ouvre l'auth Google dans la même fenêtre
  ...
  useEffect(() => {
    ...
    async function fetchBookedSlots() {
      ...
      try {
        // Récupère tous les events Google Calendar
        const response = await fetch(`${API_BASE_URL}/calendar`);
        if (!response.ok) throw new Error("Impossible de charger les disponibilités.");

        const googleEvents = await response.json();

        // Filtre les events du jour sélectionné et extrait leurs heures de début
        const occupiedHours = googleEvents
          .filter((event) => {
            // Les events Google Calendar ont start.dateTime
            const eventDate = new Date(event.start?.dateTime || event.start?.date);
            return eventDate.toISOString().startsWith(selectedDate);
          })
          .map((event) => extractHourSlot(event.start.dateTime));
  ...
  async function handleBookAppointment() {
    ...
    try {
      ...
      // 1. Crée l'event sur Google Calendar via la route /appointments (calendarRoute)
      const googleResponse = await fetch(`${API_BASE_URL}/calendar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          summary: "Consultation diététique",
      ...
      if (!googleResponse.ok) {
        ...
        throw new Error(err.error || "Erreur Google Calendar.");
      }

      const googleEvent = await googleResponse.json();

      // 2. Sauvegarde aussi en base de données via la route /appointments (appointmentRoute)
      //    On envoie vers le bon endpoint REST avec authenticateToken
      const dbResponse = await fetch(`${API_BASE_URL}/appointments`, {
        ...
        body: JSON.stringify({
          ...
          startTime,
          endTime,
          description: "Consultation diététique",
          googleEventId: googleEvent.id,
        }),
      });

      if (!dbResponse.ok) {
        // Le RDV Google est créé mais pas en BDD — on prévient sans bloquer
        console.warn("RDV créé sur Google mais non sauvegardé en base.");
      }

      setSuccessMessage(
        `Rendez-vous confirmé le ${formatDateFr(selectedDate)} à ${selectedSlot} — ${selectedSlot.replace(":00", "")}h à ${String(slotHour + 1).padStart(2, "0")}h.`
      );
  ...
  return (
    <div className="book">
      <div className="book__container">
        ...
        {/* ── Bandeau Google Calendar : statut + bouton connexion ── */}
        <div className={`book__gcal ${isGoogleConnected ? "book__gcal--ok" : "book__gcal--warn"}`}>
          ...
          {/* Bouton connexion visible uniquement pour le diété/admin */}
        ...
        {/* Formulaire de réservation (désactivé si Google non connecté) */}
                  ...
                  return (
                    <button
                      ...
                      className={`book__slot
                        ${isBooked ? "book__slot--booked" : ""}
                        ${isSelected ? "book__slot--selected" : ""}
                      `}
